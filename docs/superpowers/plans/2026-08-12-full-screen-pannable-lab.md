...
- Reuse the existing theme tokens verbatim — `--paper`, `--ink`, `--mustard`, `--teal`, `--vermilion`, `--hazmat`, `--font-display` (Bangers), `--font-body` (Space Grotesk), `--font-mono` (Space Mono). Do not add new colors or fonts.
